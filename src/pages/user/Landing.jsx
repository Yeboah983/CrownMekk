...
import { defaultSEO, pageSEO } from "../../seoConfig";

const LandingPage = () => {
  const { data: landingData, error: landingError } = useGoogleSheetData(
    "LANDINGPAGE",
    "A1:G100"
  );
  const { data: featuredData, error: featuredError } = useGoogleSheetData(
    "FEATUREDPAGE",
    "A1:G100"
  );
  ...
  const landingContent =
    landingData.find((item) => item.SECTIONTYPE?.toLowerCase() === "landing") ||
    {};
  ...
  return (
    <>
    <SEO
      title={pageSEO.home.title || defaultSEO.title}
      description={pageSEO.home.description || defaultSEO.description}
      keywords={defaultSEO.keywords}
      author={defaultSEO.author}
      url={defaultSEO.url}
      image={pageSEO.home.image || defaultSEO.image}
    />
